...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground overflow-x-hidden">
      ...
      >
        <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
          <Link href="/" className="relative z-50 flex items-center gap-3 group" data-testid="link-home">
            ...
            />
            <span className="font-display font-bold text-lg tracking-widest uppercase hidden sm:block text-foreground">
              Rachnakar
            </span>
          </Link>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 bg-background/96 backdrop-blur-xl pt-24 px-4 flex flex-col border-b border-primary/20"
          >
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`py-4 text-2xl font-display uppercase tracking-widest border-b border-primary/10 ${
      ...
      <footer className="border-t border-primary/15 bg-[rgba(248,243,234,0.95)] backdrop-blur-sm mt-auto">
        <div className="container mx-auto px-4 md:px-8 py-16">
          ...
          <div className="mt-16 pt-8 border-t border-primary/10 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-muted-foreground font-mono">
